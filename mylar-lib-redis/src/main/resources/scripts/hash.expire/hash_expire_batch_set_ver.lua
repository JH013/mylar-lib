...
-- 当HashKey不存在时设置Hash值
local function hashSetOnVersion(_hashField, _hashValueTimestamp, _hashValueVersion, _hashValueOriginalValue, _hashValueData)
    ...
    -- HashKey已过期
    if currentTime > _timestamp or v == _hashValueOriginalValue then
...
for i = 3, len, 4 do
    ...
    -- HashValue 原始版本号
    local hashValueOriginalVersion = ARGV[i + 3]

    -- HashValue 数据
    local hashValueData = ARGV[i + 4]

    -- 当HashKey不存在时设置Hash值
    ret = ret + hashSetOnVersion(hashField, hashValueTimestamp, hashValueVersion, hashValueOriginalVersion, hashValueData)
end
