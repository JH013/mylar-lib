 ...
 */
public class RedisLeakBucketRateLimiter extends AbstractRedisRateLimiter<LeakyBucketRateLimitArgs> {
     ...
     */
    public RedisLeakBucketRateLimiter(String limitKey, LeakyBucketRateLimitArgs limitArgs) {
    ...
    protected DefaultRedisScript<?> getScript() {
        return RedisRateLimiterScript.singleton().luaLeakyBucketRateLimiter();
    }
    ...
    protected List<String> getKeys() {
        ...
        if (this.limitKey.startsWith(CLUSTER_SLOT_PREFIX)) {
            return Arrays.asList(String.format("%s:leaky", this.limitKey), String.format("%s:timestamp", this.limitKey));
        }
    ...
    protected List<String> getArgs(int requestCount) {
        return Arrays.asList(
                String.valueOf(this.limitArgs.getLeakRate()),
